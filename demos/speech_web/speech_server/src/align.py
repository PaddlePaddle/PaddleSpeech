...
os.environ['PATH'] = os.path.realpath(MFA_PATH) + '/:' + os.environ['PATH']
...
def alignment(wav_path: str,
    ...
    if mfa_version == 'v1':
        ...
        # mfa 2.0
        mfa_out = os.path.join(tmpbase, "mfa_out") 
        ...
        if lang == 'en':
            ...
            exit(-1)
        CMD = 'mfa align' + ' ' + str(
            tmpbase) + ' ' + DICT + ' ' + MODEL_DIR + ' ' + str(mfa_out)
        os.system(CMD)
        tg_path = str(mfa_out) + '/' + utt + '.TextGrid'
...
def get_phns_spans(wav_path: str,
    ...
    phn, dur, w2p = alignment(
        wav_path=wav_path, text=old_str, lang=lang, fs=fs, n_shift=n_shift, mfa_version=mfa_version)
