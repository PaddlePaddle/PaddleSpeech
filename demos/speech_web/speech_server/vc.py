...
# 新建文件夹
base_sources = [
    VC_UPLOAD_PATH, VC_OUT_PATH,
    FT_UPLOAD_PATH, FT_OUT_PATH, FT_DEFAULT_PATH,
    SAT_UPLOAD_PATH, SAT_OUT_PATH,
]
...
# exp_dir = os.path.join(FT_EXP_BASE_PATH)
# ft_model.finetune(input_dir=os.path.realpath(data_path), exp_dir=os.path.realpath(exp_dir))
    ...
    for file in files:
        try:
            if 'blob' in file.filename:
                out_file_path = os.path.join(SavePath, datetime.datetime.strftime(datetime.datetime.now(), '%H%M') + randName(3) + ".wav")
            else:
...
async def VcUpload(files: List[UploadFile]):
    ...
    for file in files:
        try:
            if 'blob' in file.filename:
                out_file_path = os.path.join(VC_UPLOAD_PATH, datetime.datetime.strftime(datetime.datetime.now(), '%H%M') + randName(3) + ".wav")
            else:
            ...
            error_info = error_info + file.filename + " " + str(e) + "\n"
            continue
    return SuccessRequest(result=f"上传成功：{right}, 上传失败：{error}, 失败原因： {error_info}")
...
async def VcCloneG2P(base:VcBaseText):
    if os.path.exists(base.wavPath):
        try:
            if base.func == 'ge2e':
                wavName = base.wavName[:-4]+"_g2p.wav"
                wavPath = os.path.join(VC_OUT_PATH, wavName)
                vc_model.vc(text=base.text, input_wav=base.wavPath, out_wav=wavPath)
            else:
                wavName = base.wavName[:-4]+"_tdnn.wav"
                wavPath = os.path.join(VC_OUT_PATH, wavName)
                vc_model_tdnn.vc(text=base.text, input_wav=base.wavPath, out_wav=wavPath)
            res = {
                "wavName": wavName,
                "wavPath": wavPath
            }
            ...
            print(e)
        return ErrorRequest(message=f"克隆失败，合成过程报错: {str(e)}") 
    else:
...
async def VcCloneSAT(base:VcBaseSAT):
    # 重新整理 sat_label_dict
    if base.filename not in sat_label_dic or sat_label_dic[base.filename] != base.old_str:
    ...
    if base.language == "zh":
        ...
        if base.function == "synthesize":
            output_file_path = os.path.join(SAT_OUT_PATH, "sat_syn_zh_" + base.filename)
            # 中文克隆
            ...
            output_file_path = os.path.join(SAT_OUT_PATH, "sat_edit_zh_" + base.filename)
            # 中文语音编辑
            sat_result = sat_model.zh_synthesize_edit(
                old_str=base.old_str,
                new_str=base.new_str,
            ...
            output_file_path = os.path.join(SAT_OUT_PATH, "sat_cross_zh_" + base.filename)
            # 中文跨语言
            ...
            return ErrorRequest(message="请检查功能选项是否正确，仅支持:synthesize, edit, crossclone")
...
async def SATUpload(files: List[UploadFile]):
    ...
    for file in files:
        try:
            if 'blob' in file.filename:
                out_file_path = os.path.join(SAT_UPLOAD_PATH, datetime.datetime.strftime(datetime.datetime.now(), '%H%M') + randName(3) + ".wav")
            else:
...
async def FTUpload(base:VcBaseFT):
    try:
        ...
        return SuccessRequest(result=f"上传成功")
    except Exception as e:
        return ErrorRequest(result=f"上传失败")
...
async def FTModel(base:VcBaseFTModel):
    ...
    if not os.path.exists(data_path):
        return ErrorRequest(message=f"数据文件夹不存在")
   
    data_base = data_path.split(os.sep)[-1]
    exp_dir = os.path.join(FT_EXP_BASE_PATH, data_base)
    try:
        exp_dir = ft_model.finetune(input_dir=os.path.realpath(data_path), exp_dir=os.path.realpath(exp_dir))
        if exp_dir:
            return SuccessRequest(result=exp_dir)
        else:
            return ErrorRequest(message=f"微调失败")
    except Exception as e:
        print(e)
        return ErrorRequest(message=f"微调失败")
...
async def FTSyn(base:VcBaseFTSyn):
    try:
        if not os.path.exists(base.exp_path):
            return ErrorRequest(result=f"模型路径不存在")
        wav_name = randName(5)
        wav_path = ft_model.synthesize(text=base.text, wav_name=wav_name, out_wav_dir=os.path.realpath(FT_OUT_PATH), exp_dir = os.path.realpath(base.exp_path))
        if wav_path:
            res = {
                    "wavName": wav_name+".wav",
                    "wavPath": wav_path
                }
            return SuccessRequest(result=res)
        else:
