...
from paddlespeech.s2t.models.transducer import TransducerModel
...
class TransducerTrainer(Trainer):
    ...
    def train_batch(self, batch_index, batch_data, msg):
        ...
        # loss, attention_loss, ctc_loss, rnnt_loss = self.model(audio, audio_len, text,
        #                                             text_len)
        loss_dict = self.model(audio, audio_len, text, text_len)

        # loss div by `batch_size * accum_grad`
        loss = loss_dict['loss'] / train_conf.accum_grad
        losses_np = {'loss': float(loss_dict['loss']) * train_conf.accum_grad}
        if loss_dict['loss_att']:
            losses_np['att_loss'] = float(loss_dict['loss_att'])
        if loss_dict['loss_ctc']:
            losses_np['loss_att'] = float(loss_dict['loss_ctc'])
        if loss_dict['loss_rnnt']:
            losses_np['rnnt_loss'] = float(loss_dict['loss_rnnt'])
    ...
    def valid(self):
        ...
        for i, batch in enumerate(self.valid_loader):
            ...
            if paddle.isfinite(loss):
                ...
                if loss_dict['loss_att']:
                    valid_losses['val_att_loss'].append(
                        float(loss_dict['loss_att']))
                if loss_dict['loss_ctc']:
                    valid_losses['val_ctc_loss'].append(
                        float(loss_dict['loss_ctc']))
                if loss_dict['loss_rnnt']:
                    valid_losses['val_rnnt_loss'].append(
                        float(loss_dict['loss_rnnt']))
    ...
    def setup_model(self):
        ...
        model = TransducerModel.from_config(model_conf)
        if self.parallel:
...
class TransducerTester(TransducerTrainer):
    def __init__(self, config, args):
        super().__init__(config, args)
