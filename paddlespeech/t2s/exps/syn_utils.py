...
model_alias = {
    ...
    "diffsinger": "paddlespeech.t2s.models.diffsinger:DiffSinger",
    ...
    "pwgan_inference": "paddlespeech.t2s.models.parallel_wavegan:PWGInference",
    ...
    "erniesat": "paddlespeech.t2s.models.ernie_sat:ErnieSAT",
    "erniesat_inference": "paddlespeech.t2s.models.ernie_sat:ErnieSATInference",
}
...
def get_test_dataset(test_metadata: List[Dict[str, Any]],
    ...
    test_dataset = DataTable(data=test_metadata,
                             fields=fields,
...
def get_dev_dataloader(dev_metadata: List[Dict[str, Any]],
    ...
    # vocoder
        ...
        train_batch_fn = Clip_static(batch_max_steps=batch_max_steps,
                                     hop_size=n_shift,
...
def get_frontend(lang: str = 'zh',
    ...
    if lang == 'zh':
        ...
        frontend = SingFrontend(pinyin_phone_path=pinyin_phone,
...
def run_frontend(
    ...
    if lang == 'zh':
        ...
        input_ids = frontend.get_input_ids(svs_input=svs_input,
                                           to_tensor=to_tensor)
        phone_ids = input_ids["phone_ids"]
...
def get_am_inference(
    ...
    if am_name == 'fastspeech2':
        ...
        am = am_class(vocab_size=vocab_size,
                      tone_size=tone_size,
                      spk_num=spk_num,
                      **am_config["model"])
    elif am_name == 'tacotron2':
...
def am_to_static(am_inference,
    ...
    if am_name == 'fastspeech2':
        ...
        if am_dataset in {"aishell3", "vctk", "mix", "canton"
            ...
            am_inference = jit.to_static(am_inference,
                                         input_spec=[
                                             InputSpec([-1],
                                                       dtype=paddle.int64),
                                             InputSpec([-1], dtype=paddle.int64)
...
def voc_to_static(voc_inference,
                  voc: str = 'pwgan_csmsc',
                  inference_dir=Optional[os.PathLike]):
    voc_inference = jit.to_static(voc_inference,
                                  input_spec=[
...
def get_predictor(
    ...
    config = inference.Config(str(Path(model_dir) / model_file),
...
def get_streaming_am_output(input: str,
    ...
    phone_ids = frontend_dict['phone_ids']
    phones = phone_ids[0].numpy()
    am_encoder_infer_output = get_am_sublayer_output(am_encoder_infer_predictor,
                                                     input=phones)

    am_decoder_output = get_am_sublayer_output(am_decoder_predictor,
                                               input=am_encoder_infer_output)

    am_postnet_output = get_am_sublayer_output(am_postnet_predictor,
                                               input=np.transpose(
                                                   am_decoder_output,
                                                   (0, 2, 1)))
