...
class G2PWOnnxConverter:
    def __init__(self,
        ...
        self.config = load_config(config_path=os.path.join(
            uncompress_path, 'config.py'),
                                  use_default=True)
        ...
        self.polyphonic_chars = [
            line.split('\t')
            for line in open(polyphonic_chars_path,
        ...
        self.monophonic_chars = [
            line.split('\t')
            for line in open(monophonic_chars_path,
                             encoding='utf-8').read().strip().split('\n')
        ]
        ...
        with open(os.path.join(uncompress_path, 'char_bopomofo_dict.json'),
                  'r',
                  encoding='utf-8') as fr:
    ...
    def __call__(self, sentences: List[str]) -> List[List[str]]:
        ...
        onnx_input = prepare_onnx_input(tokenizer=self.tokenizer,
                                        labels=self.labels,
        ...
        preds, confidences = predict(session=self.session_g2pW,
