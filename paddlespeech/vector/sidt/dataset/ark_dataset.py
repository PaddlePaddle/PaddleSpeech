...
"""
Load feats dataset which generated by kaldi
"""
...
def get_ark_collate_fn():
    ...
    return ark_collate_fn


def create_kaldi_ark_dataset(cls):
    """Kaldi ark,scp dataset factory
    ...
    class KaldiArkDataset(cls):
        """
        Dataset used to load kaldi ark/scp files.
        """
        def __init__(self, scp_file, label2utt, min_item_size=1,
                     max_item_size=1, repeat=1, min_chunk_size=200,
                     max_chunk_size=400, select_by_speaker=False):
            ...
            self._collate_fn = get_ark_collate_fn()
...
if __name__ == "__main__":
    ...
    try:
        ...
        train_dataset = create_kaldi_ark_dataset(torch.utils.data.Dataset)(train_scp_file, train_spk2utt, repeat=100)
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=5,
                                                   shuffle=False, num_workers=1,
                                                   collate_fn=train_dataset.collate_fn)
        test_dataset(train_loader)

        dev_dataset = create_kaldi_ark_dataset(torch.utils.data.Dataset)(train_scp_file, train_spk2utt,
                                                                         select_by_speaker=False)
        dev_loader = torch.utils.data.DataLoader(dataset=dev_dataset, batch_size=5,
                                                   shuffle=False, num_workers=1,
                                                   collate_fn=train_dataset.collate_fn)
    ...
    try:
        ...
        train_dataset = create_kaldi_ark_dataset(paddle.io.Dataset)(train_scp_file, train_spk2utt, repeat=100)
        train_loader = paddle.io.DataLoader(dataset=train_dataset, batch_size=5,
                                            return_list=True, shuffle=False,
                                            places=place, num_workers=1,
                                            collate_fn=train_dataset.collate_fn)
        test_dataset(train_loader)

        dev_dataset = create_kaldi_ark_dataset(paddle.io.Dataset)(train_scp_file, train_spk2utt,
                                                                  select_by_speaker=False)
        dev_loader = paddle.io.DataLoader(dataset=dev_dataset, batch_size=5,
                                            return_list=True, shuffle=False,
                                            places=place, num_workers=1,
                                            collate_fn=train_dataset.collate_fn)
        test_dataset(dev_loader)
