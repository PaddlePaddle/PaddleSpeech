...
def load_data_csv(csv_path, replacements={}):
    ...
    with open(csv_path, newline="") as csvfile:
        ...
        # variable_finder 开始匹配从$开始，到反斜杠/结束
...
def read_audio(waveforms_obj):
    ...
    if isinstance(waveforms_obj, str):
        audio, _ = sound_file_load(waveforms_obj)
    ...
    duration = (stop - start) / audio_info.samplerate 
    start = start / audio_info.samplerate
    audio, fs = sound_file_load(path, duration=duration, offset=start)
    # default we transform the wave sample point from int16 to float32
    audio = depth_convert(audio, "float32")
    return audio
...
def read_audio_multichannel(waveforms_obj):
    ...
    if isinstance(waveforms_obj, str):
        audio, _ = sound_file_load(waveforms_obj)
        audio = depth_convert(audio, "float32").transpose(1, 0)
        return audio
    ...
    for f in files:
        audio_info = soundfile.info(f)
        # start = start / audio_info.samplerate
        # duration = num_frames / audio_info.samplerate
        audio, fs = sound_file_load(
            f, duration=num_frames / audio_info.samplerate, 
            offset=start / audio_info.samplerate)
        audio = depth_convert(audio, "float32")
        waveforms.append(audio)
    out = np.concatenate(waveforms, axis=0)
...
def write_audio(filepath, audio, samplerate):
    ...
    if isinstance(audio, paddle.Tensor):
        audio = audio.numpy()
    save_wav(audio, samplerate, filepath)
...
def length_to_mask(length, max_len=None, dtype=None, device=None):
    ...
    if max_len is None:
        max_len = length.max().astype("int64").item()  # using arange to generate mask
    # 这里 expand 是有问题的
    mask = paddle.arange(
