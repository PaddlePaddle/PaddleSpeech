...
# 动态元素：这个类使用来对数据转换
# 根据注释可以动态加载，不需要一次性加载到内存中，这样可以节省内存和显存
# 但是会出现大量的机器I/O操作
class DynamicItem:
...
def takes(*argkeys):
    ...
    def decorator(obj):
        ...
        if isinstance(obj, DynamicItem):
            # print("takes decorator")
            if obj.takes:
                raise ValueError("Can't overwrite DynamicItem.takes")
            # print("obj: {} input: {}".format(obj, argkeys))
            obj.takes = argkeys
            return obj
...
def provides(*output_keys):
    ...
    def decorator(obj):
        ...
        if isinstance(obj, DynamicItem):
            ...
            # print("fun{} output: {}".format(obj, output_keys))
...
class DataPipeline:
    ...
    def add_static_keys(self, static_keys):
        ...
        """
        # 将所有的静态元素注册到图中
        # 每个静态元素一个图节点，将原始的静态元素名称和图节点关联起来
        for key in static_keys:
            # print("add static key to data pipeline: {}".format(key))
    ...
    def set_output_keys(self, keys):
        ...
        # 依赖图的处理顺序，在第一次处理的时候，需要生成该处理顺序，一旦生成之后就会缓存起来
        self._exec_order = None
    ...
    def compute_outputs(self, data):
        ...
        # print("exec order: {}".format(self._exec_order))
    ...
    def _compute(self, data, order, output_mapping):
        ...
        # print("compute result: {}".format(compute_result))
