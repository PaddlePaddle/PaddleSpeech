...
# 一个依赖图的节点，包含依赖图的关键字名称key, 边edges，以及该节点的数据data
# 边edges表示的是目标节点列表
...
class DependencyGraph:
    ...
    def __init__(self):
        self.digraph = [] # 图中节点的树木
        self.key2ind = {} # 对图中每个关键字进行编号
    ...
    def add_node(self, key=None, data=None):
        ...
        if key in self.key2ind:  # Implicitly added already; don't add again.
            ...
            return key
        # 对每个关键字进行编号
        # 对每个关键字创建一个依赖图节点DGNode
        # 然后编号表示的就是其对应的依赖图中的节点
        # 这里没有使用字典映射，而是使用通过数组中的下标进行映射表示
    ...
    def add_edge(self, from_key, to_key):
        ...
        # 获取源节点的所有目标节点，如果不在其中，则将其添加到目标节点中
