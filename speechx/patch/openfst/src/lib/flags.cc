...
static void SetProgSrc(const char *src) {
    ...
    // stdin and stdout into "binary" mode, so that 0x0A won't be translated
    // into
...
#endif
    // Remove "-main" in src filename. Flags are defined in fstx.cc but
    // SetFlags()
...
void SetFlags(const char *usage,
    ...
    for (; index < *argc; ++index) {
        ...
        while (argval[0] == '-')
            argval = argval.substr(1);  // Removes initial '-'.
        string arg = argval;
        string val = "";
