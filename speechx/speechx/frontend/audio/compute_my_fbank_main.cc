...
DEFINE_int32(num_bins, 64, "fbank num bins");
...
int main(int argc, char* argv[]) {
    ...
    // opt.frame_opts.samp_freq = 32000;
    opt.frame_opts.samp_freq = 16000;
    opt.frame_opts.frame_length_ms = 32;
    opt.frame_opts.frame_shift_ms = 10;
    opt.mel_opts.num_bins = 64;
    // opt.mel_opts.low_freq = 50;
    // opt.mel_opts.high_freq = 14000;
    ...
    ppspeech::FeatureCache feature_cache(feat_cache_opts, std::move(fbank));
    LOG(INFO) << "fbank: " << true;
    ...
    for (; !wav_reader.Done() && !wav_info_reader.Done();
        ...
        // printf("%f %f %f %f %f %f\n", waveform(0), waveform(1), waveform(3),
        //                             waveform(tot_samples - 3), waveform(tot_samples - 2), waveform(tot_samples - 1));
